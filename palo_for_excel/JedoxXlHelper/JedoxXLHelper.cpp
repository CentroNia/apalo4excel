...
namespace jedox {
	namespace palo {
		...
		ServerSPtr JedoxXLHelper::getServer(const std::string& host, const unsigned int port, const std::string& user, const std::string& password, std::string& key, jedox::palo::ServerProtocol protocol) {
		...
		void JedoxXLHelper::SetVersionInfo(const std::string& version) {
			...
			m_JedoxXLHelperImpl->desc.append("OS 5.0");
