...
namespace Palo {
	namespace Types {
		std::string SourceLocation::GetLocation() const {
			std::ostringstream ss;
			ss << file << ':' << line;
			return ss.str();
		}

		const std::string& CPException::GetType() const {
			static const std::string type = "Exception";
			return type;
		}

		const std::string& CPException::GetMessage() const {
			return msg;
		}

		std::string CPException::GetLocation() const {
