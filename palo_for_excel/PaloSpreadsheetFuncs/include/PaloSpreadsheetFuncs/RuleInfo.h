...
namespace Palo {
	namespace Types {
		...
		struct RuleInfo {
			...
			RuleInfo( const jedox::palo::RULE_INFO& ri ) : identifier( ri.identifier ), definition( ri.definition ), extern_id( ri.extern_id ), comment( ri.comment ), timestamp( ri.timestamp ), activated( ri.activated ) {}
