...
namespace Palo {
	namespace Types {
		...
		struct DimensionElementInfoSimple {
			...
			DimensionElementInfoSimple() : name(""), type(DimensionElementType::Numeric), identifier(-1) {
			}
		};
		 ...
		 */
		struct ConsolidationElementInfo 
			: public DimensionElementInfoSimple {
				...
				ConsolidationElementInfo() : DimensionElementInfoSimple(), weight(1.0) {}
				
				ConsolidationElementInfo(const DimensionElementInfoSimple& deis, double w) : DimensionElementInfoSimple(deis), weight(w) {}
			double weight;
		};
		...
		struct DimensionElementInfo : public DimensionElementInfoSimple {
			...
			DimensionElementInfo() : level(0), indent(0), depth(0), position(0), num_parents(0), num_children(0), locked(false) {
			}
		};
		...
		struct ParentElementInfo {
			...
			ParentElementInfo() : identifier(-1) {
			}

			ParentElementInfo(long int id) : identifier(id) {
			}
		...
		struct ChildElementInfo {
			...
			ChildElementInfo() : identifier(-1), weight(0) {
			}

			ChildElementInfo(long int id, double w) : identifier(id), weight(w) {
			}
		...
		struct DimensionElementInfoReduced : public DimensionElementInfoSimple {
			...
			DimensionElementInfoReduced() : level(0), indent(0), depth(0), position(0), num_parents(0), num_children(0), locked(false) {
			}
			
			DimensionElementInfoReduced(const jedox::palo::ELEMENT_INFO &ei) : DimensionElementInfoSimple( ei.nelement, ei.type, ei.element){
				size_t i;
				level = ei.level;
				indent = ei.indent;
				depth = ei.depth;
