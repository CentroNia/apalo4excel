...
namespace Palo {
	namespace SpreadsheetFuncs {
		...
		public:
			GenericArrayBuilderImpl() : ref_count( 0 ) {}
		...
		public:
			GenericArrayBuilder( GenericArrayBuilderImpl* builder ) : real_builder( builder ) {
				real_builder->addRef();
			}

			GenericArrayBuilder( const GenericArrayBuilder& other ) : real_builder( other.real_builder ) {
				real_builder->addRef();
			}
