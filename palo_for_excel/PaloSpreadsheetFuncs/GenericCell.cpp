...
StringArrayArray GenericCell::getStringArrayArray() {
	StringArrayArray saa;

	saa.reserve( getArray().minRemaining() );

	for ( GenericCell::Iterator i = getArray(); !i.end(); ++i )
		saa.push_back( i->getStringArray() );
...
DimensionElementType GenericCell::getDimensionElementType() {
	const std::string s = StringUtils::toUpper( getString() );
	if ( s == "N" )
...
jedox::palo::SPLASH_MODE GenericCell::getSplashMode() {
	if ( getType() == GenericCell::TString ) {
		std::string s = StringUtils::toUpper( getString() );
...
GenericCell& GenericCell::set( const LicenseInfo& li ) {
	...
	a.append( "name", a.createGenericCell()->set( li.name ) );
	a.append( "users", a.createGenericCell()->set( li.users ) );
	a.append( "expiredate", a.createGenericCell()->set( (unsigned long)li.expiredate ) );
	a.append( "type", a.createGenericCell()->set( li.type ) );
...
boost::shared_ptr<jedox::palo::Server> GenericCell::getConnection( ConnectionPool& cp ) {
	return cp.get( getString() );
}
