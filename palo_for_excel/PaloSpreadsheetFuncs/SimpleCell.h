	...
	namespace SpreadsheetFuncs {
		namespace SimpleCell {
			...
			public:
				...
				virtual boost::shared_ptr<jedox::palo::Server> getConnection( ConnectionPool& cp, std::string& database );
			...
			public:
				...
				boost::shared_ptr<jedox::palo::Server> getConnection( ConnectionPool& cp, std::string& database );
                bool empty(bool allelements = true);
				Type getType();
