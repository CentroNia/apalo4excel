...
#include <PaloSpreadsheetFuncs/ElementListArray.h>
#include <PaloSpreadsheetFuncs/InvalidRequestException.h>

using namespace Palo::Types;
using namespace Palo::SpreadsheetFuncs;

StringArrayArray ElementListArray::getStringArrayArray() const {
	StringArrayArray sa;

	// empty sub-array means "all elements"

	for ( ElementListArray::const_iterator i = begin(); i != end(); i++ ) {
		if ( i->all() ) {
			sa.push_back( StringArray() );
		} else {
