	...
	namespace XLL {
		...
		public:
			...
			struct CellID {
				...
				/*! hash_map support */
				bool operator < (const XLCellRange::CellID& right) const;
			};
			//typedef typename CellID_ CellID;
			
			XLCellRange( const XLCellRange& other );
			XLCellRange( XLOPERX ref );
			XLCellRange( LPXLOPERX ref );
			XLCellRange( const CellID& ref );
			~XLCellRange();
			...
			*/
			static bool getCallerCheck(bool dothrow, XLOPERX *xCaller);
			static XLCellRange getCallerProcess(XLOPERX *xCaller);
			...
			XLCellRange::Color getColor() const;
			...
			static CellID getCallerID(void *dummy = NULL);
			CellID getCellID();
		...
		private:
			...
			XLREFX getXLREF() const;
		};

		inline bool XLCellRange::CellID::operator < ( const  XLCellRange::CellID& right) const {
			const XLCellRange::CellID& left = *this;
		...
		inline XLCellRange::CellID::operator size_t() const {
			
			return rw + col * XLCell::max_rows + sheetID * XLCell::max_rows * XLCell::max_cols;
		}
	}
}
#endif
