...
LPXLOPERX TempStr(LPSTR str, void (*free_func)(void *), int dll_free) {
	...
	oper = (LPXLOPERX)calloc(1, sizeof(XLOPERX));
	if (oper == NULL) {
		return NULL;
	}
...
char **get_string_array(unsigned int *len, short int *is_vertical, LPXLOPERX oper) {
	...
	switch(oper->xltype) {
		case xltypeSRef:
		case xltypeRef:
			if (Excelx(xlCoerce, &temp, 1, oper) != xlretSuccess) {
				return NULL;
			}
			a = get_string_array(len, is_vertical, &temp);
			Excelx(xlFree, 0, 1, &temp);
			return a;
		case xltypeMulti:
...
int get_double(LPXLOPERX oper, double *d, short int get_upper_left, short int from_str) {
	...
	switch(oper->xltype) {
		...
		case xltypeStr:
			if (from_str != 0) {
				if (Excelx(xlCoerce, &temp, 2, oper, TempInt(xltypeNum)) != xlretSuccess) {
					return XL_FAILURE;
				}
...
char *create_cleartext_key_current() {
	...
	key = create_cleartext_key((LPXLOPERX)&xCaller);

	Excelx(xlFree, NULL, 1, (LPXLOPERX)&xCaller);

	return key;
}

char *create_cleartext_key(LPXLOPERX xCaller) {
	char *name = NULL, *result = NULL, *cell = NULL;
	size_t len = 0;
...
char *get_book_sheet(LPXLOPERX xCaller, short int current) {
	...
	retcode = Excelx(xlSheetNm,(LPXLOPERX)&xRes,1,xCaller);
	...
	name = get_string((LPXLOPERX)&xRes);
...
char *get_cell(LPXLOPERX xCaller) {
	...
	if (xCaller->xltype == xltypeSRef) {
		ref = xCaller->val.sref.ref;
	} else {
		ref = *xCaller->val.mref.lpmref->reftbl;
	}

	// create string
	len = _scprintf("%d%d", ref.rwFirst + 1, ref.rwLast + 1);
	result = (char*)calloc(len + 6, sizeof(char));
	if ( result == NULL) {
		return NULL;
	}
	t = result;
