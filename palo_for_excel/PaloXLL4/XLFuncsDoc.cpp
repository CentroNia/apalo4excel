...
inline std::string Trim(const std::string& mesg) {
...
inline std::string CheckLen(const std::string& mesg, size_t maxlen) {
	std::string tmpstr(Trim(mesg));
	return (tmpstr.length() >= maxlen) ? Trim(tmpstr.substr(0, maxlen)) : tmpstr;
}			

inline std::string CheckCodePage(const std::wstring& mesg) {
	std::string tmpstr;

	char *s = NULL;
	unicode_win_err err = wcs2utf8(&s, mesg.c_str(), mesg.size());

	if ((err != UNICODE_WIN_SUCCESS) || (s == NULL)) {
		tmpstr = "";
	} else {
		tmpstr = Trim(s);
	}
...
#ifdef __XLL_VERSION_4
	return Trim(XLSpecialConv::utf82local(tmpstr));
#else
	return tmpstr;
#endif
}
...
StringArray XLFuncsDoc::getDecl( const Palo::SpreadsheetFuncs::FunctionDocumentation& fd ) const {
	...
	static const size_t max_args = XL_FUNC_MAX_ARGS;
	static const size_t max_desc_length = XL_FUNC_MAX_DESC_LENGTH;
	static const size_t max_short_desc_length = XL_FUNC_MAX_SHORT_DESC_LENGTH;
	static const size_t max_effective_args_to_describe = XL_EFFECTIVE_ARGS_TO_DESCRIBE_MAX;
	static const size_t effective_max_args = std::min(max_args - 10, max_effective_args_to_describe );

	static const std::string lang = XLRegistry::getLanguage();
	static const std::string emptyString = "";

	std::vector<std::string> decl;

	decl.push_back("xll_" + fd.c_name);
...
//	static const std::string param_type2 = PARAM2_TYPE;
	...
	decl.push_back(param_types);

	decl.push_back(fd.xl_doc.xl_name);
	...
	for ( PrototypeDocumentation::ArgKeys::const_iterator i = p.arg_keys.begin(); i != p.arg_keys.end(); ++i ) {
		...
		std::string t, desc(CheckCodePage(a.short_desc[lang]));
		...
		if (!desc.empty()) {
			...
			const size_t repeat_count = a.repeat ? effective_max_args : std::min(( size_t )( i - p.arg_keys.begin() + 1 ), effective_max_args ) ;
			for ( size_t j = i - p.arg_keys.begin(); ( param_names.size() + t.size() <= max_desc_length ) && j < repeat_count; ++j ) {
				++params_to_describe;
				param_names += t;
			} 
	...
	{
		if (decl[i].size()>max_desc_length)
		{
			decl[i] = CheckLen(decl[i], max_desc_length);
		}
	}
