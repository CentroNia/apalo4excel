...
#define STR(x) to_string<XLOPERX>(x).c_str()
...
//			LPCTSTR doc = aia.Documentation();
			Topic dc(TUID(aia.TopicId()),
				...
				.section(_T("Syntax"),
					to_string<XLOPERX>(syntax).c_str())
				.section(_T(""), table)
				/*
				.section(doc[0] == _T('<') 
					? _T("") // free form
...
write_alias(const cat_fun_map& cf)
{
	...
	for (cat_fun_citer i = cf.begin(); i != cf.end(); ++i) {
		for (fun_citer j = i->second.begin(); j != i->second.end(); ++j) {
			if (j->second->isFunction() || j->second->isMacro()) {
				alias << _T("IDH_");
				const OPERX& xProc = j->second->Procedure();
				// skip first '?' char
				alias.Write(xProc.val.str + 2, xProc.val.str[0] - 1);
				alias << _T("=html\\");
	...
	for (cat_fun_citer i = cf.begin(); i != cf.end(); ++i) {
		for (fun_citer j = i->second.begin(); j != i->second.end(); ++j) {
			if (j->second->isFunction() || j->second->isMacro()) {
				...
				map.Write(xProc.val.str + 2, xProc.val.str[0] - 1);
				map << _T(" ");
				TCHAR buf[32];
		...
		do {
			if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
				if (!dotdot(fd.cFileName))
					ENSURE (rmdir(DIR(dir, fd.cFileName)));
			}
			else {
...
{
	std::basic_string<TCHAR> pfiles = Environment::Variable<TCHAR>(_T("ProgramFiles")); //!!!"ProgramFiles(x86)"
...
xll_make_doc(void)
{
	...
	try {
		...
		for (AddInX::addin_iter i = l.begin(); i != l.end(); ++i) {
			if ((*i)->Arg().TopicId()) {
				ensure ((*i)->Arg().Category().xltype == xltypeStr);
				cf_map[(*i)->Arg().Category()][(*i)->Arg().Sort()] = &((*i)->Arg());

				write_aml((*i)->Arg());
				write_xmlcomp((*i)->Arg()); // companion file
			}
		}
...
xai_make_doc(void)
{
	Register<XLOPER>("_xll_make_doc@0", "XLL.DOC");
...
xai_unmake_doc(void)
{
	Unregister<XLOPERX>(Args("_xll_make_doc@0", "XLL.DOC"));
