...
//      Revisions for Palo (c) Junction BI LLC:
//		2014-03-14 Add sorted collection to SelectedNodes
//		2014-03-23 Added WndProc monitor for removed nodes to remove from selection
//		2014-03-22 Added NodesCollection constructor to copy from a TreeNodeCollection
//		2014-03-21 Corrected IsPlusMinusClicked check
//		2013-12-17 Added AllowSingleSelectedNode for compatibility, expose Select/Unselect functions
//		2013-11-11 Allow single selection to use SelectedNode
//
// *****************************************************************************
...
namespace JControls {
	...
	/// <summary>
	/// The MSTreeView control is a regular treeview with multi-selection capability.
	/// </summary>
	[ToolboxItem(true)]
	public class MSTreeView : System.Windows.Forms.TreeView
	{
		...
		/// </summary>
		protected Hashtable htblSelectedNodes = new Hashtable();
		...
		protected override void WndProc(ref Message m)
			{
			if (m.Msg == 0x1101 /* TVN_DELETEITEM */)
				{
				//	Node deleted - insure it is not referenced anymore
				//	Note: This has potential to be optimized
				TreeNode removed = null;
				foreach (TreeNode n in htblSelectedNodes.Values)
					if (n.TreeView == null || (this.IsHandleCreated && n.Handle == m.LParam))
						{
						removed = n;
						break;
						}
				if (removed != null)
					htblSelectedNodes.Remove(removed.GetHashCode());
				}
			base.WndProc(ref m);
			}
		...
		public new TreeNode SelectedNode
		{
			get
			{
				...
				else if (AllowSingleSelectedNode || selectionMode == TreeViewSelectionMode.SingleSelect)
					{
					//	Compatability use of single node:
					foreach (TreeNode n in htblSelectedNodes.Values)
						return n;
					return base.SelectedNode;
					}
			...
			set
			{
			if (!blnInternalCall && selectionMode != TreeViewSelectionMode.SingleSelect)
				{
					//	Compatability use of single node:
					if (AllowSingleSelectedNode)
						{
						UnselectAllNodesExceptNode(value, TreeViewAction.Unknown);
						if (value != null) SelectNode(value, true, TreeViewAction.Unknown);
						}
					else
						throw new NotSupportedException("Use SelectedNodes instead of SelectedNode.");
				}
		...
		/// <summary>
		/// Allows access to a single selected node for compatability
		/// </summary>
		[Browsable(true)]
		[Category("Behavior")]
		[DesignerSerializationVisibility(DesignerSerializationVisibility.Visible)]
		public bool AllowSingleSelectedNode {get; set;}
		...
		public NodesCollection SelectedNodes
		{
			get
			{
				...
				// JMM 04-03-14 Add sorted collection
				TreeNode[] selected = new TreeNode[htblSelectedNodes.Count];
				int i=0;
				foreach (TreeNode tn in htblSelectedNodes.Values)
					{selected[i++] = tn;}
				Array.Sort(selected, new Comparison<TreeNode>(compareNodesIndex));
				NodesCollection selectedNodesCollection = new NodesCollection(selected);
		...
		private int compareNodesIndex(TreeNode A, TreeNode B)
			{
			if (A.Parent != B.Parent)
				{
				if (A == B.Parent) return -1;
				if (B == A.Parent) return 1;
				return compareNodesIndex(A.Parent ?? A, B.Parent ?? B);
				}
		...
		public void UnselectAllNodes(TreeViewAction tva=TreeViewAction.Unknown)
		{
		...
		/// <param name="tva">Specifies the action that caused the selection change.</param>
		public void UnselectAllNodesExceptNode(TreeNode nodeKeepSelected, TreeViewAction tva)
		{
			// First, build list of nodes that need to be unselected
			ArrayList arrNodesToDeselect = new ArrayList();
			foreach (TreeNode selectedTreeNode in htblSelectedNodes.Values)
		...
		public bool SelectNode(TreeNode tn, bool select, TreeViewAction tva)
		{
		...
		{
			//	JMM 03-21-14: Leaf nodes don't have +/-
			if (tn.Nodes.Count == 0)
				return false;
		...
		{
			//JMM 03-21-14 Allow dragging node that isn't selected
			NodesCollection nc = this.SelectedNodes;
			if (e.Item is TreeNode && !nc.Contains((TreeNode)e.Item))
				nc = new NodesCollection((TreeNode)e.Item);
			e = new ItemDragEventArgs(MouseButtons.Left, nc);
			base.OnItemDrag(e);
		}
	...
	public class NodesCollection : CollectionBase
	{
		...
		/// <summary>Constructor for an array of treenodes</summary>
		/// <param name="addnodes"></param>
		public NodesCollection(params TreeNode[] addnodes)
			{
			foreach (TreeNode n in addnodes)
				List.Add(n);
			}

		/// <summary>Constructor to copy from a TreeNodeCollection</summary>
		/// <param name="nc"></param>
		public NodesCollection(TreeNodeCollection nc)
			{
