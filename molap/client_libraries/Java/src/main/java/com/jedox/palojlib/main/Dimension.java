...
public class Dimension extends CachedComponent implements IDimension{
	...
	public void addAttributeValues(IAttribute attribute, IElement[] elements,Object[] values) throws PaloJException, PaloException {
		...
		if(this.attributesNameMap.get(att.getName().toLowerCase()) == null){
			IAttribute serverAtt = getAttributeByName(att.getName());
			if(serverAtt == null)
  ...
  /***************************************************************************************/
	...
	protected synchronized void buildCache(boolean withAttributes) throws PaloException, PaloJException{
		...
		if(checkAttributes){
			...
			for(IElement att:attributesArray){
				...
				this.attributesNameMap.put(((Element)att).getName().toLowerCase(), (Element)att);
			}
		}
	...
	@Override
	public String[] getGlobalSubsetVariables(String subsetname) {
		return dimensionhandler.getGlobalSubsetVariables(this.database.getId(),this.name,subsetname);
	}


	@Override
	public IElement[] evaluateGlobalSubset(String subsetname, IVariable[] subsetVariables) {
		return dimensionhandler.evaluateGlobalSubset(this.database,this,subsetname,subsetVariables);
	}
	
	@Override
	public String[] getGlobalSubsets() {
		
		ICube cube = this.database.getCubeByName(subsetCube);
		
		IElement [][] paths = new IElement[2][];	
		IElement internDim = cube.getDimensionByName("#_DIMENSION_").getElementByName(getName(), false);
