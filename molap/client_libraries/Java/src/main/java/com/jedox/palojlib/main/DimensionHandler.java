...
public final class DimensionHandler{
	...
	private static String VIEW_CALCULATE_REQUEST = "/view/calculate?";
	...
	public String[] getGlobalSubsetVariables(int databaseid, String dimname, String subsetname) {
		int major = HttpHandlerManager.getInstance().getHttpHandlerInfo(contextId).majorVersion;
		boolean apiAllowed = (major>=6);
		if(!apiAllowed)
			throw new PaloJException("This API is allowed up olap server verion 6.0");
		
		StringBuilder currentRequest = getGlobalSubsetRequestCommon(databaseid,dimname,subsetname);

		try {
			HttpHandlerManager.getInstance().getHttpHandler(contextId).send(currentRequest,true,false);
			throw new PaloJException("This should never happen.");
		} catch (PaloException e) {
			if(e.getCode().equals("1035")){
	...
	private StringBuilder getGlobalSubsetRequestCommon(int databaseid, String dimname, String subsetname) {
		StringBuilder VIEW_CALCULATE_REQUEST_BUFFER = new StringBuilder(VIEW_CALCULATE_REQUEST);
		return VIEW_CALCULATE_REQUEST_BUFFER.append("database=").append(databaseid).append("&view_subsets=subsetName;").append(dimname).append(";6;").append(subsetname).append(";1;-1&view_axes=$$subsetName;;;1");
	}

	public IElement[] evaluateGlobalSubset(Database db, Dimension dim,
			String subsetname, IVariable[] subsetVariables) {
		...
		if(subsetVariables!=null && subsetVariables.length!=0){
			...
			for(IVariable var:subsetVariables){
				variables.append(var.getName()+"="+ Helpers.urlEncode(var.getValue()) + ";");
			}
			variables = variables.deleteCharAt(variables.length()-1);
			varString = subsetVariables.length + ";" + variables;	
		}
		
		StringBuilder currentRequest = getGlobalSubsetRequestCommon(db.getId(),dim.getName(),subsetname);
		currentRequest = new StringBuilder(currentRequest.toString().replace("-1", varString));
		
		String[][] response = HttpHandlerManager.getInstance().getHttpHandler(contextId).send(currentRequest,true,false);
		ArrayList<Element> result = new ArrayList<Element>();
		for(int i=0;i<response.length;i++){
			if(!response[i][0].trim().isEmpty()){
				response[i][0] = response[i][0].substring(response[i][0].indexOf('\n')+1);
				int elementId;
				try {
					elementId = Integer.parseInt(response[i][0]);
				} catch (NumberFormatException e1) {
					// this happen when no elements exists
					continue;
				}			
				String name = response[i][1];
				ElementType type = parseType(Integer.parseInt(response[i][6]));
				Element e = new Element(contextId, db, dim.getId(), elementId, name, type,0,new int[0],new int[0],new double[0],null,null,-1,null,true);
				result.add(e);
			}
		}
