...
public final class DatabaseHandler{
	...
	protected void addDimension(Database database, String name, DimensionType type) throws PaloException, PaloJException{

		StringBuilder CREATE_DIMENSION_REQUEST_BUFFER = new StringBuilder(CREATE_DIMENSION_REQUEST);
		StringBuilder currentRequest = CREATE_DIMENSION_REQUEST_BUFFER.append("new_name=").append(Helpers.urlEncode(name)).append("&database=").append(database.getId()).append("&type=").append(type.ordinal());
		HttpHandlerManager.getInstance().getHttpHandler(contextId).send(currentRequest,true,false);
	}

	protected LinkedHashMap<String, Dimension> getDimensions(Database database) throws PaloException, PaloJException{
		...
		LinkedHashMap<String,Dimension> dimensions = new LinkedHashMap<String,Dimension>();
		for(int i=0;i<responses.length;i++){
			...
			dimensions.put(responses[i][1].toLowerCase(),new Dimension(contextId, Integer.parseInt(responses[i][0]),responses[i][1],database,type,responses[i][7],responses[i][8],responses[i][3],responses[i][4],responses[i][5],responses[i][10],responses[i][2]));
		}
		return dimensions;
	}

	protected void addCube(Database database, String name, int[] dimensionIds, CubeType type) throws PaloException, PaloJException{
		...
		StringBuilder currentRequest = CREATE_CUBE_REQUEST_BUFFER.append("new_name=").append(Helpers.urlEncode(name)).append("&database=").append(database.getId()).append("&type=").append(type.ordinal()).append("&dimensions=").append(dimensions);
		HttpHandlerManager.getInstance().getHttpHandler(contextId).send(currentRequest,true,false);
	}


	protected LinkedHashMap<String, Cube> getCubes(IDatabase database) throws PaloException, PaloJException{
		...
		String[][] responses = HttpHandlerManager.getInstance().getHttpHandler(contextId).send(currentRequest,true,false);
		LinkedHashMap<String,Cube> cubes = new LinkedHashMap<String,Cube>();
		...
		for(int i=0;i<responses.length;i++){
			...
			cubes.put(responses[i][1].toLowerCase(),new Cube(contextId, Integer.parseInt(responses[i][0]),responses[i][1],databaseObj, info));
		}
		return cubes;
	}
	...
	public void save(int databaseId,boolean complete) throws PaloException, PaloJException {
		StringBuilder DATABASE_SAVE_REQUEST_BUFFER = new StringBuilder(DATABASE_SAVE_REQUEST);
		StringBuilder currentRequest = DATABASE_SAVE_REQUEST_BUFFER.append("database=").append(databaseId).append("&complete=").append(complete?"1":"0");
		HttpHandlerManager.getInstance().getHttpHandler(contextId).send(currentRequest,true,false);
