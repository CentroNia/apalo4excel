...
public class HttpParser {
	...
	public final String[] parseLine(String str,char delim) {
		...
		String[] possibleEntries = str.trim().split(String.valueOf(delim),-1);// maximum number of entries
		...
		for(String possibleEntry:possibleEntries){
			if(!possibleEntry.contains("\"") && entry.length()==0){// if it is the first one and no double quotes exists in it, then add it directly
				...
				if(numberOfDoubleQuotes%2 == 0 && entry.length()==0){
					...
					entry.append(possibleEntry + String.valueOf(delim));// an open quote is somewhere before, this should be completly added to the temp string
				}else if(numberOfDoubleQuotes%2 != 0 && entry.length()==0){
					entry.append(possibleEntry.substring(1, possibleEntry.length()) + String.valueOf(delim));
				}else{
