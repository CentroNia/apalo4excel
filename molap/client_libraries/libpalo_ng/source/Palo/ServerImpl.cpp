...
bool ServerImpl::tryGetElements(boost::shared_ptr<PaloClient> paloClient, unsigned int db, unsigned int dim, IdentifierType parentid, std::list<ELEMENT_INFO> &result)
{
	DimKey dimKey = std::make_pair(db, dim);
	boost::unique_lock<boost::mutex> lock(m_Lock);
	std::map<std::pair<unsigned int, unsigned int>, boost::shared_ptr<CacheItemBase> >::iterator it = m_Caches.find(dimKey);
	...
	for (std::vector<ElementCache>::const_iterator it = res->begin(); it != res->end(); ++it) {
		bool valid = true;
		if (parentid != (IdentifierType)-2) {
			if (parentid == (IdentifierType)-1) {
				valid = !it->number_parents;
			} else {
				valid = std::find(it->parents.begin(), it->parents.end(), parentid) != it->parents.end();
			}
		}
		if (valid) {
			result.push_back(*it);
