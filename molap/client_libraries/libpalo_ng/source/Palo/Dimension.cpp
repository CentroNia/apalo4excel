...
std::list<ELEMENT_INFO> Dimension::getElements(IdentifierType parentid, IdentifierType &start, const std::string &element, IdentifierType limit) const
{
	...
	if (limit) {
		if (limit != (IdentifierType)-1 || start || !element.empty() || !m_ServerImpl->tryGetElements(m_PaloClient, m_Database, m_Dimension, parentid, ret)) {
			IdentifierType e = (IdentifierType)-1;
			if (!element.empty()) {
