...
std::string StringUtils::getLocale()
{
	std::string coll = setlocale(LC_COLLATE, 0);
	if (coll.empty() || coll == "C") {
		coll = setlocale(LC_ALL, 0);
	} else {
		U_NAMESPACE_QUALIFIER Locale l(coll.c_str());
		coll = l.getName();
	}
	if (coll.empty() || coll == "C") {
		coll = U_NAMESPACE_QUALIFIER Locale::getDefault().getName();
	}
	return coll;
}
