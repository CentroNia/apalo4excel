...
static inline void getHeaders(jedox::palo::HEADER_LIST& headerList, std::iostream& stream)
{
...
static inline jedox::palo::CLIENT_RESPONSE_APTR sendRequest(boost::shared_ptr<std::iostream> stream, const jedox::palo::HttpClientRequest& clientRequest, jedox::palo::HttpClient::ClientMode mode, bool &outClosesConnection, const std::string &Target, unsigned int TargetPort)
{
	static const char CRLF[] = "\r\n";
	static const char END_OF_HEADER[] = "\r\n";

	const jedox::palo::Url& url = clientRequest.getUrl();
	...
	const jedox::palo::HttpClientRequest::Method method = clientRequest.getMethod();
	switch (method) {
	case jedox::palo::HttpClientRequest::POST:
		/*	The POST method is used to request that the origin server accept the entity enclosed in the request as a new
		 subordinate of the resource identified by the Request-URI in the Request-Line. */
		break;

	case jedox::palo::HttpClientRequest::GET:
	case jedox::palo::HttpClientRequest::HEAD:
		if (method == jedox::palo::HttpClientRequest::GET) {
	...
	default:
		jedox::palo::HttpExceptionFactory::raise(jedox::palo::HttpExceptionFactory::HTTP_CLIENT_UNDEFINED_REQUEST);
	}
	...
	(*stream) << "Content-Length: " << (clientRequest.getMethod() == jedox::palo::HttpClientRequest::POST ? url.getQuerySize() : 0) << CRLF;
	...
	//if (mode & jedox::palo::HttpClient::PERFORM_AUTHENTICATION && url.isAuthenticatable()) {
	//	(*stream) << jedox::palo::BasicAuthentication(url.getUsername(), url.getPassword()).getHeader();
	//}
	...
	if (mode & jedox::palo::HttpClient::PERFORM_KEEP_ALIVE) {
	...
	for (jedox::palo::HEADER_LIST::const_iterator it = clientRequest.getHeaderList().begin(); it != clientRequest.getHeaderList().end(); ++it) {
	...
	if (clientRequest.getMethod() == jedox::palo::HttpClientRequest::POST) {
		(*stream) << url.getQuery();
	}
	...
	if (header.size() > 2) {
		...
		statusCode = jedox::util::lexicalConversion(unsigned int, std::string, header[1]);
		for (std::vector<std::string>::iterator it = header.begin() + 2; it < header.end(); ++it) {
			statusText.append(*it);
		}
	} else {
		// TODO Not an valid HTTP-Response header
		jedox::palo::HttpExceptionFactory::raise(jedox::palo::HttpExceptionFactory::HTTP_CLIENT_INVALID_RESPONSEHEADER);
	}

	// Get response headers. See the documentation in the getHeaders function
	jedox::palo::HEADER_LIST headerList;
	...
	if (responseHasBody) {
		...
		if (headerList.find("transfer-encoding") != headerList.end()) {
			...
			contentLength = jedox::util::lexicalConversion(size_t, std::string, headerList["content-length"]);
	...
	return jedox::palo::CLIENT_RESPONSE_APTR(new jedox::palo::HttpClientResponse(clientRequest, protocolVersionText, statusCode, headerList, body));;
}


static inline jedox::palo::CLIENT_RESPONSE_APTR sendHttpsRequest(const jedox::palo::HttpClientRequest& clientRequest, jedox::palo::HttpClient::ClientMode mode, const std::string &Target, unsigned int TargetPort)
{

	static jedox::palo::HttpsClientConnectionPool &connectionPool = jedox::palo::HttpsClientConnectionPool::instance();
	std::unique_ptr<jedox::palo::HttpsClientConnection> connection = connectionPool.adoptClientConnection(clientRequest.getUrl());

	if (!connection->hasValidCertificate()) {
		jedox::palo::PaloExceptionFactory::raise(jedox::palo::PaloExceptionFactory::ERROR_SSL_FAILED, "Invalid server certificate.", "Server returned invalid certificate.");
	}

	bool closesConnection;
	jedox::palo::CLIENT_RESPONSE_APTR res(sendRequest(connection->getStream(), clientRequest, mode, closesConnection, Target, TargetPort));
...
static inline jedox::palo::CLIENT_RESPONSE_APTR sendHttpRequest(const jedox::palo::HttpClientRequest& clientRequest, jedox::palo::HttpClient::ClientMode mode, const std::string &Target, unsigned int TargetPort)
{

	static jedox::palo::HttpClientConnectionPool &connectionPool = jedox::palo::HttpClientConnectionPool::instance();
	std::unique_ptr<jedox::palo::HttpClientConnection> connection = connectionPool.adoptClientConnection(clientRequest.getUrl());
