	...
	m_Hostname(hostname), m_Port(port), m_hasValidCertificate(false)
{
	...
	BIO_set_conn_hostname(m_Bio, (hostname + ":" + util::lexicalConversion(std::string, unsigned int, port)).c_str());
	...
	if (m_hasValidCertificate) {
		...
		if (cert) {
			...
			if (boost::regex_search(certname, what, boost::regex("CN=([^/]*)(/|$)")) && what.size() > 1) {
				std::string compname(what[1].first, what[1].second);
				std::string s = boost::regex_replace(compname, boost::regex("\\."), "\\\\.");
				s = boost::regex_replace(s, boost::regex("\\*"), ".*");
				boost::match_results<std::string::const_iterator> m;
				if (!boost::regex_match(m_Hostname, m, boost::regex(s, boost::regex::icase), boost::match_all)) {
