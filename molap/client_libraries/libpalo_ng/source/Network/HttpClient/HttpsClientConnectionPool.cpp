...
struct HttpsClientConnectionPool::HttpsClientConnectionPoolImpl {
	...
	void returnClientConnection(std::unique_ptr<HttpsClientConnection> &clientConnection)
	{
		...
		key.append(clientConnection->getHostname()).append(seperator).append(util::lexicalConversion(std::string, unsigned int, clientConnection->getPort()));
		{
	...
	std::unique_ptr<HttpsClientConnection> adoptClientConnection(const Url& url)
	{
		...
		std::string key;
		std::string seperator(KEY_SEPERATOR);
		key.append(url.getHostname()).append(seperator).append(util::lexicalConversion(std::string, unsigned int, url.getPort()));
