...
std::string SpreadsheetFuncsBase::FPaloElementSibling(boost::shared_ptr<Server> s, const std::string& database, const std::string& dimension, const std::string& element, int sibling_offset)
{
	using namespace jedox::palo;
	...
	if (ei.number_parents == 0) {
		std::unique_ptr<DimensionElementInfoSimpleArray> root = FPaloDimensionSimpleTopListElements(s, database, dimension);
		DimensionElementInfoSimpleArray::const_iterator it;
		for (it = root->begin(); it != root->end(); ++it) {
			if (it->identifier == ei.element) {
				break;
			}
		}
		if (it == root->end()) {
		...
		unsigned int pos = it - root->begin();
		if ((sibling_offset > 0 && pos + sibling_offset >= root->size()) || (sibling_offset < 0 && (signed)pos < -sibling_offset)) {
			throw InvalidRequestException(CurrentSourceLocation);
		} else {
			return root->at(pos + sibling_offset).name;
		}
	} else {
